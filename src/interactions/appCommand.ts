				...
				embeds: [
					new EmbedBuilder()
						...
						.addFields(
							...
							{ name: 'サブコマンド', value: interaction.options.getSubcommand(false) || 'なし' },
							{
								name: 'ユーザー',
								value:
									interaction.user.discriminator === '0'
										? `${interaction.user.globalName}(@${interaction.user.username}, ${interaction.user.id})`
										: `${interaction.user.username}#${interaction.user.discriminator}(表示名：${
												interaction.user.globalName || 'なし'
										  }, ${interaction.user.id})`,
							},
							{
								name: 'サーバー',
