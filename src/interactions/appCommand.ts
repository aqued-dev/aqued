import {
  ...
  export default async function (interaction: BaseInteraction) {
	...
	if (interaction.isChatInputCommand()) {
	  const command = botData.commands.chatInput.find(
		(value) => value.name === interaction.commandName,
	  );
	  if (!command)
		  ...
		  async () =>
			await botData.commandExecutors.number.set(
			  ...
			  (await botData.commandExecutors.number.get(interaction.commandName))
				? Number(
					await botData.commandExecutors.number.get(
					  interaction.commandName,
					),
				  ) + 1
				: 1,
	  ...
	  if (
		command.data.ownersOnly &&
		!botData.owners.includes(interaction.user.id)
	  )
		return await interaction.error(
		  "あなたはこのコマンドを利用できません",
		  "管理者専用コマンドです",
	  ...
	  if (command.data.permissions) {
		...
		if (
		  !authorPerms ||
		  !command.data.permissions.every((permission) =>
			authorPerms.has(permission),
		  )
		) {
		...
		if (
		  !botPerms ||
		  !command.data.permissions.every((permission) =>
			botPerms.has(permission),
		  )
		) {
		  const permission: PermissionFlags[] = command.data.permissions;
		  return await interaction.error(
			"権限不足",
			"このコマンドを実行するためには、Botに`" +
			  translatePermission(permission).join(", ") +
			  "`の権限が必要です。",
			true,
		  );
	  ...
	  const command = botData.commands.userCotextMenu.find(
		(value) => value.name === interaction.commandName,
	  );
	  ...
	  const command = botData.commands.messageCotextMenu.find(
		(value) => value.name === interaction.commandName,
	  );
