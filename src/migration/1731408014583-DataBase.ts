...
export class DataBase1731408014583 implements MigrationInterface {
	name = 'DataBase1731408014583';

	public async up(queryRunner: QueryRunner): Promise<void> {
		await queryRunner.query(
			`CREATE TABLE \`CHANNEL_SETTING\` (\`CHANNEL_ID\` bigint NOT NULL COMMENT 'チャンネルID', \`GLOBAL_CHAT\` tinyint NULL COMMENT 'グローバルチャット参加の有無', \`AUTO_NEWS\` tinyint NULL COMMENT 'ニュース自動公開の有無', \`FORCE_PIN\` json NULL COMMENT 'メッセージを下に固定', PRIMARY KEY (\`CHANNEL_ID\`)) ENGINE=InnoDB`
		);
		await queryRunner.query(
			`CREATE TABLE \`ERROR\` (\`id\` bigint NOT NULL COMMENT 'エラーSnowFlake', \`text\` text NOT NULL COMMENT 'エラー内容', PRIMARY KEY (\`id\`)) ENGINE=InnoDB`
		);
		await queryRunner.query(
		...
		await queryRunner.query(
			`CREATE TABLE \`GUILD_SETTING\` (\`GUILD_ID\` bigint NOT NULL COMMENT 'ギルドID', \`AUTO_MODS\` text NULL COMMENT 'Aquedにより設定されたAutoModのId配列', \`WELCOME_MESSAGE\` json NULL COMMENT 'ウェルカムメッセージ', \`LEAVE_MESSAGE\` json NULL COMMENT '退出メッセージ', PRIMARY KEY (\`GUILD_ID\`)) ENGINE=InnoDB`
		);
		await queryRunner.query(
